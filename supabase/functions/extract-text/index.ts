...
function isLikelyPhone(line: string): boolean {
  return /(\+?\d[\d\s\-()]{7,}\d)/.test(line) || /(05\d{8})/.test(line) || /(\+966\d{9})/.test(line) || /(\+20\d{10})/.test(line) || /(\+971\d{9})/.test(line) || /(\+\d{1,3}[\s\-]?\d{6,})/.test(line);
}
...
function isLikelyLocation(line: string): boolean {
  return /(riyadh|jeddah|dammam|mecca|medina|ksa|saudi|saudi arabia|الرياض|جدة|الدمام|مكة|المدينة|السعودية|المملكة العربية السعودية|cairo|alexandria|dubai|abu dhabi|القاهرة|الإسكندرية|دبي|أبوظبي|amman|beirut|doha|muscat|kuwait|manama|عمان|بيروت|الدوحة|مسقط|الكويت|المنامة)/i.test(line);
}

function isLikelyContactLine(line: string): boolean {
  return isLikelyEmail(line) || isLikelyPhone(line) || isLikelyLinkedIn(line) || isLikelyWebsite(line) || isLikelyLocation(line);
}
...
function looksLikeRoleHeader(line: string): boolean {
  ...
  const hasRoleWords = /(manager|engineer|specialist|supervisor|analyst|consultant|developer|officer|lead|administrator|coordinator|trainer|director|architect|managerial|designer|accountant|teacher|nurse|doctor|technician|intern|assistant|associate|executive|representative|secretary|clerk|receptionist|مدير|مهندس|أخصائي|مشرف|محلل|استشاري|مطور|مسؤول|قائد|منسق|مدرب|محاسب|معلم|ممرض|طبيب|فني|متدرب|مساعد|سكرتير|موظف)/i.test(t);
  const hasCompanyWords = /(company|corp|inc|llc|ltd|group|hospital|ministry|university|school|agency|solutions|network|technologies|systems|consulting|services|شركة|مؤسسة|مجموعة|مستشفى|وزارة|جامعة|معهد|تقنية)/i.test(t);
...
function isProbablyName(line: string): boolean {
  ...
  if (/^(summary|experience|education|skills|certifications|projects|languages|الملخص|الخبرة|التعليم|المهارات|الشهادات|المشاريع|اللغات)$/i.test(t)) return false;
...
function evaluateQuality(text: string, fileSize: number): { quality: string; score: number } {
  ...
  const printable = cleaned.replace(/[^\x20-\x7E\u0600-\u06FF\u0750-\u077F\u08A0-\u08FF\uFB50-\uFDFF\uFE70-\uFEFF\n\t]/g, "");
...
const COMMON_SECTION_HEADINGS: Record<string, StructuredKey> = {
  "job title": "job_title", title: "job_title", position: "job_title", headline: "job_title",
  contact: "contact_info", "contact information": "contact_info", "contact info": "contact_info",
  "contact details": "contact_info", "personal information": "contact_info", "personal details": "contact_info",
  summary: "summary", "professional summary": "summary", profile: "summary",
  "professional profile": "summary",
  about: "summary", "about me": "summary", objective: "summary", "career objective": "summary",
  overview: "summary", "executive summary": "summary", "career summary": "summary",
  experience: "work_experience", "work experience": "work_experience",
  "employment history": "work_experience", employment: "work_experience",
  "professional experience": "work_experience", "career history": "work_experience",
  "work history": "work_experience", "relevant experience": "work_experience",
  skills: "skills", "technical skills": "skills", "core skills": "skills",
  "key skills": "skills", competencies: "skills", "core competencies": "skills",
  "professional skills": "skills", "skills & competencies": "skills",
  "areas of expertise": "skills", "technical competencies": "skills",
  ...
  courses: "certifications", training: "certifications", "professional development": "certifications",
  "training & certifications": "certifications", "certifications & training": "certifications",
  "professional certifications": "certifications",
  projects: "projects", project: "projects", "key projects": "projects",
  "notable projects": "projects", "selected projects": "projects",
  languages: "languages", language: "languages", "language skills": "languages",
  "language proficiency": "languages",
  // Arabic
  "المسمى الوظيفي": "job_title", الوظيفة: "job_title", المنصب: "job_title",
  "بيانات الاتصال": "contact_info", "معلومات الاتصال": "contact_info",
  "البيانات الشخصية": "contact_info", "المعلومات الشخصية": "contact_info",
  الملخص: "summary", نبذة: "summary", "الملخص المهني": "summary", "الهدف الوظيفي": "summary",
  "نبذة مهنية": "summary", "نبذة عني": "summary", "ملخص مهني": "summary",
  "الخبرة العملية": "work_experience", الخبرات: "work_experience",
  "الخبرات العملية": "work_experience", "الخبرة المهنية": "work_experience",
  "تاريخ التوظيف": "work_experience",
  المهارات: "skills", "المهارات التقنية": "skills", الكفاءات: "skills",
  "المهارات المهنية": "skills", "المهارات الأساسية": "skills",
  التعليم: "education", المؤهلات: "education", "المؤهلات العلمية": "education",
  "الخلفية الأكاديمية": "education", "المؤهلات الأكاديمية": "education",
  الشهادات: "certifications", الدورات: "certifications",
  "الدورات التدريبية": "certifications", الاعتمادات: "certifications",
  "الشهادات المهنية": "certifications", "التطوير المهني": "certifications",
  المشاريع: "projects", اللغات: "languages",
};
...
function formatEducationText(raw: string): string {
  ...
  for (const line of lines) {
    ...
    const headerLike = looksLikeDateRange(line) || /\b(Bachelor|Master|Diploma|PhD|degree|University|College|بكالوريوس|ماجستير|دبلوم|جامعة|كلية)\b/i.test(line);
    if (!current) { current = line; continue; }
    if (headerLike && current.length > 0) { groups.push(current.trim()); current = line; }
    else { current += ` | ${line}`; }
  }
...
async function extractDOCXText(uint8: Uint8Array): Promise<string> {
  ...
  return combined || xmlContent.replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim();
}
...
function heuristicSectionDetection(text: string, language: "ar" | "en"): StructuredResume {
  ...
  const phoneMatch = text.match(/(\+?\d[\d\s\-()]{7,}\d)|(05\d{8})|(\+966\d{9})|(\+20\d{10})|(\+971\d{9})|(\+\d{1,3}[\s\-]?\d{6,})/g) || [];
  const linkedInMatch = text.match(/https?:\/\/(www\.)?linkedin\.com\/[^\s]+/gi) || [];
  const locationMatch = text.match(/(?:الرياض|جدة|الدمام|مكة|المدينة|Riyadh|Jeddah|Dammam|Mecca|Medina|Saudi Arabia|المملكة العربية السعودية|Cairo|Alexandria|Dubai|Abu Dhabi|القاهرة|الإسكندرية|دبي|أبوظبي|Amman|Beirut|Doha|Muscat|Kuwait|Manama)[^,\n]*/gi) || [];
  ...
  for (let i = 0; i < topLines.length; i++) {
    ...
    if (result.full_name && !result.job_title && isProbablyJobTitle(line) && !contactSet.has(line.toLowerCase().trim())) {
  ...
  console.log(`[extract-text] Phase 3: found ${boundaries.length} section boundaries: ${boundaries.map(b => `${b.field}@${b.startIdx}`).join(", ")}`);
  ...
  if (!result.work_experience) {
    const expCandidates = lines.filter((l) =>
      (looksLikeRoleHeader(l) || /^[•▪*\-]/.test(l)) &&
      !contactSet.has(l.toLowerCase().trim())
    );
  ...
  if (!result.skills) {
    const skillCandidates = lines.filter((l) => {
      ...
      return /(excel|power bi|sql|python|react|typescript|javascript|java|oracle|aws|azure|network|communication|leadership|problem solving|docker|kubernetes|git|agile|scrum|إكسل|باور بي آي|sql|oracle|تحليل|إدارة|قيادة|شبكات|برمجة)/i.test(l);
    });
  ...
  console.log(`[extract-text] Final result: name="${processed.full_name}", title="${processed.job_title}", contact_items=${splitLines(processed.contact_info).length}, exp_blocks=${processed.work_experience.split(/\n{2,}/).filter(Boolean).length}, skills_count=${splitLines(processed.skills).length}`);
