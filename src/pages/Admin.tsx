...
import {
  ArrowLeft, Users, FileText, BarChart3, Mail, Sparkles, Search,
  ChevronDown, ChevronUp, Eye, Clock, Pencil, Trash2, CreditCard,
  Building2, Upload, Download, Plus, Coins, UserPlus, KeyRound, History,
} from "lucide-react";
...
const Admin = () => {
  ...
  const [metrics, setMetrics] = useState<AdminMetrics>({ totalUsers: 0, totalResumes: 0, totalAnalyses: 0, totalGenerated: 0, totalEmails: 0, recentSignups: 0 });
  const [users, setUsers] = useState<UserRow[]>([]);
  const [activity, setActivity] = useState<ActivityItem[]>([]);
  const [searchQuery, setSearchQuery] = useState("");
  const [sortField, setSortField] = useState<"created_at" | "resumeCount" | "analysisCount" | "points_balance">("created_at");
  ...
  const [companyForm, setCompanyForm] = useState({ name: "", industry: "", email: "", website: "", location: "", contact_person: "", phone: "", notes: "" });
  ...
  const fetchAdminData = async () => {
    ...
    const activityItems: ActivityItem[] = [
      ...resumes.map(r => ({ type: "resume" as const, user_email: emailMap.get(r.user_id) || null, title: r.file_name, created_at: r.created_at })),
      ...analyses.map(a => ({ type: "analysis" as const, user_email: emailMap.get(a.user_id) || null, title: `Score: ${a.overall_score}`, created_at: a.created_at })),
      ...generated.map(g => ({ type: "generated" as const, user_email: emailMap.get(g.user_id) || null, title: g.title, created_at: g.created_at })),
      ...emails.map(e => ({ type: "email" as const, user_email: emailMap.get(e.user_id) || null, title: e.subject, created_at: e.created_at })),
  ...
  const handleAddUser = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("admin-manage-user", {
        body: { action: "create_user", email: newUserEmail.trim(), password: newUserPassword, full_name: newUserName.trim() || undefined },
      });
  ...
  const handleInspect = async (userId: string) => {
    ...
    const [profileRes, resumesRes, analysesRes, generatedRes, emailsRes, txRes] = await Promise.all([
      ...
      supabase.from("resumes").select("id, file_name, file_type, language, created_at").eq("user_id", userId).order("created_at", { ascending: false }),
      supabase.from("analyses").select("id, overall_score, language, created_at").eq("user_id", userId).order("created_at", { ascending: false }),
      supabase.from("generated_resumes").select("id, title, language, created_at").eq("user_id", userId).order("created_at", { ascending: false }),
      supabase.from("marketing_emails").select("id, subject, job_title, industry, created_at").eq("user_id", userId).order("created_at", { ascending: false }),
  ...
  const handleDownloadTemplate = () => {
    const templateData = [
      { name: "Example Company", industry: "Technology", email: "hr@example.com", website: "https://example.com", location: "Riyadh", contact_person: "Ahmed", phone: "+966500000000", notes: "" },
    ];
  ...
  const handleExcelUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (error) {
        ...
        toast.success(ar ? `تم إضافة ${toInsert.length} شركة بنجاح` : `${toInsert.length} companies added successfully`);
        fetchCompanies();
      }
    } catch {
  ...
  const handleAddCompany = async () => {
    ...
    if (error) {
      ...
      setCompanyForm({ name: "", industry: "", email: "", website: "", location: "", contact_person: "", phone: "", notes: "" });
      fetchCompanies();
    }
  };
  ...
  const activityIcon = (type: string) => {
    switch (type) {
      ...
      case "analysis": return <BarChart3 size={14} className="text-primary" />;
      case "generated": return <Sparkles size={14} className="text-primary" />;
  ...
  const planLabel = (plan: string) => {
    switch (plan) {
      case "basic": return ar ? "أساسي" : "Basic";
      case "pro": return ar ? "احترافي" : "Professional";
      case "publish_only": return ar ? "نشر فقط" : "Publish Only";
      case "enterprise": return ar ? "مؤسسات" : "Enterprise";
      default: return ar ? "مجاني" : "Free";
    }
  };
  ...
  const statusLabel = (status: string) => {
    switch (status) {
      ...
      default: return t.admin.inactive;
    }
  };

  const planBadgeClass = (plan: string) => {
    switch (plan) {
      case "basic": return "bg-blue-500/10 text-blue-600";
      case "pro": return "bg-primary/10 text-primary";
      case "publish_only": return "bg-orange-500/10 text-orange-600";
      case "enterprise": return "bg-accent/10 text-accent-foreground";
  ...
  const txTypeLabel = (type: string) => {
    const labels: Record<string, string> = ar
      ? { admin_credit: "إضافة من المدير", admin_debit: "خصم من المدير", admin_set: "تعيين رصيد", purchase: "شراء", analysis: "تحليل", enhancement: "تحسين", marketing_send: "إرسال تسويقي" }
      : { admin_credit: "Admin Credit", admin_debit: "Admin Debit", admin_set: "Balance Set", purchase: "Purchase", analysis: "Analysis", enhancement: "Enhancement", marketing_send: "Marketing Send" };
  ...
  const SortIcon = ({ field }: { field: typeof sortField }) => (
    sortField === field ? (sortAsc ? <ChevronUp size={14} /> : <ChevronDown size={14} />) : <ChevronDown size={14} className="opacity-30" />
  );
    ...
    <div className="min-h-screen bg-background" dir={ar ? "rtl" : "ltr"}>
      ...
      <main className="container py-8 md:py-12">
        ...
        <div className="flex gap-2 mb-6">
          ...
          <Button variant={activeTab === "companies" ? "default" : "outline"} onClick={() => setActiveTab("companies")} className="gap-2">
            <Building2 size={16} />
        ...
        {activeTab === "users" ? (
            ...
            <div className="lg:col-span-2 bg-card rounded-xl border border-border overflow-hidden">
              <div className="px-6 py-4 border-b border-border flex items-center gap-4 flex-wrap">
                ...
                <div className="relative w-64">
                  ...
                  <Input placeholder={t.admin.searchPlaceholder} value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-9 h-9 text-sm bg-background" />
                </div>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-border bg-muted/50">
                        <th className="text-start px-4 py-3 font-display font-medium text-foreground">{t.admin.name}</th>
                        <th className="text-start px-4 py-3 font-display font-medium text-foreground">{t.admin.email}</th>
                        <th className="text-start px-4 py-3 font-display font-medium text-foreground">{t.admin.subscription}</th>
                        <th className="text-center px-4 py-3 font-display font-medium text-foreground cursor-pointer select-none" onClick={() => handleSort("points_balance")}>
                          <span className="flex items-center justify-center gap-1"><Coins size={12} />{ar ? "النقاط" : "Points"}<SortIcon field="points_balance" /></span>
                        </th>
                        <th className="text-start px-4 py-3 font-display font-medium text-foreground cursor-pointer select-none" onClick={() => handleSort("created_at")}>
                          <span className="flex items-center gap-1">{t.admin.joined}<SortIcon field="created_at" /></span>
                        </th>
                        <th className="text-center px-4 py-3 font-display font-medium text-foreground">{t.admin.actions}</th>
                      </tr>
                    </thead>
                    <tbody>
                      {filteredUsers.map((u) => (
                        <tr key={u.user_id} className="border-b border-border last:border-0 hover:bg-muted/30">
                          <td className="px-4 py-3 font-body text-foreground">
                            <div>
                              ...
                              <span className={`text-xs px-1.5 py-0.5 rounded-full font-medium ${u.role === "admin" ? "bg-primary/10 text-primary" : "bg-secondary text-muted-foreground"}`}>
                                {u.role}
                              </span>
                            </div>
                          </td>
                          ...
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-1.5">
                              <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${planBadgeClass(u.subscription_plan)}`}>
                                {planLabel(u.subscription_plan)}
                              </span>
                              <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${statusBadgeClass(u.subscription_status)}`}>
                                {statusLabel(u.subscription_status)}
                              </span>
                            </div>
                          </td>
                          <td className="px-4 py-3 font-body text-foreground text-center font-semibold">
                            <span className={u.points_balance > 0 ? "text-primary" : u.points_balance < 0 ? "text-destructive" : "text-muted-foreground"}>
                              {u.points_balance}
                            </span>
                          </td>
                          <td className="px-4 py-3 font-body text-muted-foreground text-xs">{new Date(u.created_at).toLocaleDateString()}</td>
                          <td className="px-4 py-3">
                            <div className="flex items-center justify-center gap-1">
                              <Button variant="ghost" size="icon" onClick={() => handleInspect(u.user_id)} className="h-7 w-7" title={t.admin.inspectUser}>
                                <Eye size={14} />
                              </Button>
                              <Button variant="ghost" size="icon" onClick={() => handleOpenPoints(u)} className="h-7 w-7" title={ar ? "إدارة النقاط" : "Manage Points"}>
                                <Coins size={14} />
                              </Button>
                              <Button variant="ghost" size="icon" onClick={() => handleOpenSubscription(u)} className="h-7 w-7" title={t.admin.activateSubscription}>
                                <CreditCard size={14} />
                              </Button>
                              <Button variant="ghost" size="icon" onClick={() => handleEditUser(u)} className="h-7 w-7" title={t.admin.editUser}>
                                <Pencil size={14} />
                              </Button>
                              <Button variant="ghost" size="icon" onClick={() => setDeleteUserId(u.user_id)} className="h-7 w-7 text-destructive hover:text-destructive" title={t.admin.deleteUser}>
                                <Trash2 size={14} />
                              </Button>
                            </div>
                          </td>
                  ...
                  activity.map((item, i) => (
                    <div key={i} className="px-4 py-3 flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground">{item.user_email} · {activityLabel(item.type)}</p>
                      </div>
                      <span className="text-xs text-muted-foreground whitespace-nowrap">{new Date(item.created_at).toLocaleDateString()}</span>
                    </div>
                  ))
                )}
              </div>
          ...
          <div className="bg-card rounded-xl border border-border overflow-hidden">
            <div className="px-6 py-4 border-b border-border flex flex-wrap items-center gap-3">
              <h2 className="font-display font-semibold text-foreground">{ar ? "إدارة الشركات" : "Companies Management"}</h2>
              <div className="flex-1" />
              <div className="relative w-64">
                <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" />
                <Input placeholder={ar ? "بحث عن شركة..." : "Search companies..."} value={companySearch} onChange={e => setCompanySearch(e.target.value)} className="pl-9 h-9 text-sm bg-background" />
              </div>
              ...
              <div>
                <input ref={companyFileRef} type="file" accept=".xlsx,.xls" className="hidden" onChange={handleExcelUpload} />
                <Button variant="outline" size="sm" className="gap-2" onClick={() => companyFileRef.current?.click()} disabled={companyUploading}>
                  <Upload size={14} />
            ...
            {filteredCompanies.length === 0 ? (
              <div className="py-12 text-center">
                ...
                <p className="text-sm text-muted-foreground font-body">{ar ? "لا توجد شركات بعد" : "No companies yet"}</p>
              </div>
            ) : (
              <div className="overflow-x-auto">
                <Table>
                          ...
                          <div className="flex items-center justify-center gap-1">
                            <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => {
                              ...
                              setCompanyForm({
                                ...
                                contact_person: c.contact_person || "", phone: c.phone || "", notes: c.notes || "",
                              });
                            }}>
                              <Pencil size={14} />
                            </Button>
                            <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive hover:text-destructive" onClick={() => setDeleteCompanyId(c.id)}>
                              <Trash2 size={14} />
                            </Button>
                          </div>
                        </TableCell>
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4">
            <div>
              ...
              <Input value={newUserName} onChange={e => setNewUserName(e.target.value)} placeholder={ar ? "الاسم الكامل" : "Full Name"} />
            </div>
            <div>
            ...
            <div>
              <label className="text-sm font-medium text-foreground mb-1 block">{ar ? "كلمة المرور" : "Password"} *</label>
      ...
      {/* Inspect User Dialog */}
      <Dialog open={!!inspectUser} onOpenChange={() => { setInspectUser(null); setInspectData(null); }}>
            ...
            <div className="space-y-6">
              <div className="p-4 bg-secondary/30 rounded-lg">
                ...
                <div className="grid grid-cols-2 gap-2 text-sm">
                  <div><span className="text-muted-foreground">{t.admin.name}:</span> <span className="text-foreground">{inspectData.profile?.display_name || "—"}</span></div>
                  <div><span className="text-muted-foreground">{t.admin.email}:</span> <span className="text-foreground">{inspectData.profile?.email || "—"}</span></div>
                  <div><span className="text-muted-foreground">{t.admin.phone}:</span> <span className="text-foreground">{inspectData.profile?.phone || "—"}</span></div>
                  <div><span className="text-muted-foreground">{t.admin.joined}:</span> <span className="text-foreground">{inspectData.profile?.created_at ? new Date(inspectData.profile.created_at).toLocaleDateString() : "—"}</span></div>
                </div>
              </div>
              ...
              <Tabs defaultValue="transactions">
                <TabsList className="flex-wrap">
                  ...
                  <TabsTrigger value="resumes">{t.admin.resumes} ({inspectData.resumes.length})</TabsTrigger>
                  <TabsTrigger value="analyses">{t.admin.analyses} ({inspectData.analyses.length})</TabsTrigger>
                  <TabsTrigger value="generated">{t.admin.generated} ({inspectData.generated.length})</TabsTrigger>
                  <TabsTrigger value="emails">{t.admin.emails} ({inspectData.emails.length})</TabsTrigger>
                </TabsList>
                <TabsContent value="transactions">
                  {inspectData.transactions.length === 0 ? <p className="text-sm text-muted-foreground py-4">{t.admin.noData}</p> : (
                    <div className="space-y-2">
                      ...
                      {inspectData.transactions.map((tx: any) => (
                        <div key={tx.id} className="flex items-center justify-between p-3 bg-secondary/20 rounded-lg text-sm">
                          <div>
                ...
                <TabsContent value="resumes">
                  {inspectData.resumes.length === 0 ? <p className="text-sm text-muted-foreground py-4">{t.admin.noData}</p> : (
                    <div className="space-y-2">{inspectData.resumes.map((r: any) => (
                      <div key={r.id} className="flex items-center justify-between p-3 bg-secondary/20 rounded-lg text-sm">
                ...
                <TabsContent value="analyses">
                  {inspectData.analyses.length === 0 ? <p className="text-sm text-muted-foreground py-4">{t.admin.noData}</p> : (
                    <div className="space-y-2">{inspectData.analyses.map((a: any) => (
                      <div key={a.id} className="flex items-center justify-between p-3 bg-secondary/20 rounded-lg text-sm">
                ...
                <TabsContent value="generated">
                  {inspectData.generated.length === 0 ? <p className="text-sm text-muted-foreground py-4">{t.admin.noData}</p> : (
                    <div className="space-y-2">{inspectData.generated.map((g: any) => (
                      <div key={g.id} className="flex items-center justify-between p-3 bg-secondary/20 rounded-lg text-sm">
                ...
                <TabsContent value="emails">
                  {inspectData.emails.length === 0 ? <p className="text-sm text-muted-foreground py-4">{t.admin.noData}</p> : (
                    <div className="space-y-2">{inspectData.emails.map((e: any) => (
                      <div key={e.id} className="flex items-center justify-between p-3 bg-secondary/20 rounded-lg text-sm">
                        <div>
                          <span className="font-body text-foreground">{e.subject}</span>
                          <span className="text-xs text-muted-foreground ml-2">{e.job_title} · {e.industry}</span>
                        </div>
      ...
      <Dialog open={!!editUserId} onOpenChange={() => setEditUserId(null)}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>{ar ? "تعديل بيانات المستخدم بما فيها البريد وكلمة المرور" : "Edit user data including email and password"}</DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div>
              <label className="text-sm font-medium text-foreground mb-1 block">{t.admin.displayName}</label>
            ...
            <div>
              ...
              <Input type="password" value={editPassword} onChange={e => setEditPassword(e.target.value)} placeholder={ar ? "اتركها فارغة إذا لا تريد تغييرها" : "Leave empty to keep current"} />
            </div>
          </div>
          <DialogFooter>
        ...
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              {" · "}
              {ar ? "الرصيد الحالي:" : "Current balance:"} <span className="font-bold text-primary">{pointsUserBalance}</span>
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div>
            ...
            <div>
              ...
              <Input type="number" min="0" value={pointsAmount} onChange={e => setPointsAmount(e.target.value)} placeholder="0" />
            </div>
            <div>
              <label className="text-sm font-medium text-foreground mb-1 block">{ar ? "الوصف (اختياري)" : "Description (optional)"}</label>
              <Input value={pointsDescription} onChange={e => setPointsDescription(e.target.value)} placeholder={ar ? "سبب العملية" : "Reason for this action"} />
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setPointsUserId(null)}>{t.common.cancel}</Button>
            <Button onClick={handleSavePoints} disabled={!pointsAmount}>{t.admin.saveChanges}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!subUserId} onOpenChange={() => setSubUserId(null)}>
        <DialogContent className="max-w-md">
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setSubUserId(null)}>{t.common.cancel}</Button>
      ...
      <Dialog open={addCompanyOpen} onOpenChange={setAddCompanyOpen}>
        <DialogContent className="max-w-md max-h-[80vh] overflow-y-auto">
          ...
          <CompanyFormFields />
          <DialogFooter>
            <Button variant="outline" onClick={() => setAddCompanyOpen(false)}>{t.common.cancel}</Button>
            <Button onClick={handleAddCompany} disabled={!companyForm.name.trim()}>{ar ? "إضافة" : "Add"}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <AlertDialog open={!!deleteCompanyId} onOpenChange={() => setDeleteCompanyId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t.admin.confirmDelete}</AlertDialogTitle>
            <AlertDialogDescription>{ar ? "هل أنت متأكد من حذف هذه الشركة؟" : "Are you sure you want to delete this company?"}</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>{t.common.cancel}</AlertDialogCancel>
