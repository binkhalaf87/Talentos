...
const Login = () => {
  ...
  // Never call navigate() inside handleSubmit — wait for state update.
  useEffect(() => {
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <Input type="email" value={email} onChange={(e) => setEmail(e.target.value)}
              placeholder="email@example.com" required autoComplete="email" className="bg-card"/>
          </div>
          <div>
            <label className="text-sm font-display font-medium text-foreground mb-1.5 block">{t.auth.password}</label>
            <Input type="password" value={password} onChange={(e) => setPassword(e.target.value)}
              placeholder="••••••••" required autoComplete="current-password" className="bg-card"/>
          </div>
          ...
          <Button type="submit" className="w-full" disabled={submitting || googleLoading}>
            {submitting ? <><Loader2 size={14} className="animate-spin mr-2"/>{t.common.loading}</> : t.auth.login}
          </Button>
        </form>
