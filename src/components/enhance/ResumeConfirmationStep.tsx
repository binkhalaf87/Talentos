...
import {
  User, Briefcase, Mail, Phone, MapPin, Linkedin,
  CheckCircle2, AlertTriangle, Edit3, Sparkles,
} from "lucide-react";
import { cn } from "@/lib/utils";
import type { StructuredResume } from "@/lib/resume-utils";
import { parseContactString, contactFieldsToString } from "./EditableResumeHeader";
...
const ResumeConfirmationStep = ({
  structured,
  onConfirm,
  onBack,
  isRTL,
  t,
}: ResumeConfirmationStepProps) => {
  const parsed = parseContactString(structured.contact);

  const fields = [
    { icon: User, label: t("Full Name", "الاسم الكامل"), value: structured.name, key: "name" },
    { icon: Briefcase, label: t("Job Title", "المسمى الوظيفي"), value: structured.job_title, key: "job_title" },
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="text-center space-y-3">
        ...
        <p className="text-sm text-muted-foreground max-w-lg mx-auto">
          {t(
            "We've extracted the following information from your resume. Please review and correct any errors before proceeding to the editor.",
            "تم استخراج المعلومات التالية من سيرتك الذاتية. يرجى المراجعة وتصحيح أي أخطاء قبل الانتقال للمحرر.",
          )}
        </p>
      </div>
              ...
              return (
                <div key={f.key} className="flex items-center gap-3 p-3 rounded-lg bg-muted/30 border border-border">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className={cn("text-sm font-medium truncate", hasValue ? "text-foreground" : "text-muted-foreground italic")}>
