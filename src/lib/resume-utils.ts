...
export function cleanResumeText(raw: string): string {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!endsWithPunctuation && !nextIsBullet && !lineIsHeading && nextStartsLower && line.length < 80 && nextLine.length > 0) {
...
export function parseExperienceIntoBullets(experienceText: string): ExperienceRole[] {
  ...
  const groups = experienceText.split(/\n{2,}/).map(g => g.trim()).filter(Boolean);
  return groups.map((group, gIdx) => {
    const lines = group.split("\n").map(l => l.trim()).filter(Boolean);
    const header = lines[0] || "";
    const bullets = lines.slice(1).map((line, bIdx) => ({
...
export function isLikelyPhone(line: string): boolean {
  return /(\+?\d[\d\s\-()]{7,}\d)/.test(line) || /(05\d{8})/.test(line) || /(\+966\d{9})/.test(line) || /(\+20\d{10})/.test(line) || /(\+971\d{9})/.test(line);
}
...
export function isLikelyLocation(line: string): boolean {
  return /(riyadh|jeddah|dammam|ksa|saudi|location|address|الرياض|جدة|الدمام|السعودية|العنوان|الموقع|cairo|alexandria|dubai|abu dhabi|القاهرة|الإسكندرية|دبي|أبوظبي|amman|beirut|doha|muscat|kuwait|manama|عمان|بيروت|الدوحة|مسقط|الكويت|المنامة)/i.test(line);
}

export function isLikelyContactLine(line: string): boolean {
  return isLikelyEmail(line) || isLikelyPhone(line) || isLikelyLinkedIn(line) || isLikelyLocation(line) || isLikelyWebsite(line);
}
...
export function looksLikeRoleHeader(line: string): boolean {
  ...
  const hasCompanyWords =
    /(company|corp|inc|llc|ltd|group|hospital|ministry|university|school|agency|solutions|network|technologies|systems|consulting|services|تقنية|شركة|مؤسسة|مجموعة|مستشفى|وزارة|جامعة|معهد)/i.test(t);
...
function lineMatchesSection(line: string, section: keyof StructuredResume): boolean {
  ...
  if (section === "skills") {
    if (/\b(bachelor|master|diploma|phd|degree|university|college|بكالوريوس|ماجستير|دبلوم|جامعة|كلية)\b/i.test(t)) return false;
    if (looksLikeRoleHeader(line)) return false;
  }

  if (section === "education") {
    if (t.length < 30 && !looksLikeDateRange(line) && !/(bachelor|master|diploma|phd|degree|university|college|بكالوريوس|ماجستير|دبلوم|جامعة|كلية)/i.test(t)) {
      if (/(excel|power bi|sql|python|react|javascript|java|oracle|aws|network|communication|leadership|إكسل|برمجة)/i.test(t)) return false;
    }
  }
...
export function formatExperienceText(raw: string): string {
  ...
  for (const line of lines) {
    ...
    current.push(bulletLike ? `• ${normalizeBulletLine(line)}` : `• ${normalizeBulletLine(line)}`);
  }
  flush();
  return groups
    .map((group) => {
  ...
  if (Array.isArray(value)) {
    const items = value.flatMap((item) => {
      ...
      if (typeof item === "object" && item !== null) {
        ...
        return Object.values(obj).map((v) => cleanArtifacts(String(v || ""))).filter(Boolean);
      }
      return [];
    });
...
export function mapExtractedToEditor(extracted: Record<string, unknown>): StructuredResume {
  let name = cleanArtifacts(String(extracted.full_name || extracted.name || ""));
  let jobTitle = cleanArtifacts(String(extracted.job_title || extracted.title || ""));
  ...
  if (ci && typeof ci === "object" && !Array.isArray(ci)) {
    ...
    const pieces = [cObj.email, cObj.phone, cObj.mobile, cObj.location, cObj.address, cObj.linkedin, cObj.website, cObj.github]
      .map((x) => cleanArtifacts(String(x || "")))
      .filter(Boolean);
    contact = uniqueLines(pieces).join("\n");
  } else {
    contact = cleanArtifacts(String(ci || ""));
  }

  const summary = cleanArtifacts(String(extracted.summary || extracted.professional_summary || extracted.profile || ""));
  ...
  if (Array.isArray(we)) {
    ...
    for (const role of we) {
      ...
      const title = cleanArtifacts(String(r.title || r.position || r.job_title || r.role || ""));
      const company = cleanArtifacts(String(r.company || r.organization || r.employer || ""));
      const duration = cleanArtifacts(String(r.duration || r.period || r.date || [r.start_date || r.startDate, r.end_date || r.endDate].filter(Boolean).join(" - ")));
      const descSource = r.description || r.responsibilities || r.achievements || r.details || r.bullets || "";
      let bullets: string[];
      if (Array.isArray(descSource)) {
        bullets = toBullets(descSource.map(b => cleanArtifacts(String(b))));
      } else {
        bullets = toBullets(splitLines(String(descSource)));
      }
    ...
    education = ed
      .map((e: unknown) => {
        if (typeof e === "object" && e !== null) {
          ...
          return cleanArtifacts([String(eo.degree || ""), String(eo.major || eo.field || ""), String(eo.institution || eo.university || eo.college || eo.school || ""), String(eo.year || eo.graduation_year || eo.date || "")].filter(Boolean).join(" | "));
        }
  ...
  const certifications = normalizeListField(extracted.certifications || extracted.certificates || extracted.courses || extracted.training);
    ...
    projects = pr
      .map((p: unknown) => {
        if (typeof p === "object" && p !== null) {
          ...
          return cleanArtifacts([String(po.name || po.title || ""), String(po.description || "")].filter(Boolean).join(": "));
        }
  ...
  if (!name) {
    const fallbackName = cleanArtifacts(String(extracted.candidate_name || ""));
    if (fallbackName) name = fallbackName;
  }

  return enrichStructuredResume({ name, job_title: jobTitle, contact, summary, experience, skills, education, certifications, projects, languages });
}
...
export function parseResumeTextFallback(text: string): StructuredResume {
  ...
  // ── PHASE 1: Extract contact info from full text ──────────────
  const emailMatch = text.match(/[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}/gi) || [];
  const phoneMatch = text.match(/(\+?\d[\d\s\-()]{7,}\d)|(05\d{8})|(\+966\d{9})|(\+20\d{10})|(\+971\d{9})/g) || [];
  const linkedInMatch = text.match(/https?:\/\/(www\.)?linkedin\.com\/[^\s]+/gi) || [];
  const locationMatch = text.match(/(?:الرياض|جدة|الدمام|Riyadh|Jeddah|Dammam|Saudi Arabia|Cairo|Dubai|القاهرة|دبي|Amman|Beirut|Doha|Muscat|Kuwait|عمان|بيروت|الدوحة|مسقط|الكويت)[^,\n]*/gi) || [];
  ...
  for (let i = 0; i < topLines.length; i++) {
    ...
    if (!result.name && isProbablyName(line) && !contactSet.has(line.toLowerCase().trim())) {
      result.name = line;
      continue;
    }
    if (result.name && !result.job_title && isProbablyJobTitle(line) && !contactSet.has(line.toLowerCase().trim())) {
  ...
  // ── PHASE 3: Section boundary detection ───────────────────────
  ...
  // ── PHASE 5: Handle lines before first heading ────────────────
  ...
  if (!result.skills) {
    const candidates = uniqueLines(lines.filter((l) => {
      if (contactSet.has(l.toLowerCase().trim())) return false;
      if (looksLikeRoleHeader(l)) return false;
      return /(excel|power bi|sql|python|react|typescript|javascript|java|oracle|aws|azure|network|communication|leadership|problem solving|docker|kubernetes|git|agile|scrum|إكسل|باور بي آي|تحليل|إدارة|قيادة|شبكات|برمجة)/i.test(l);
    }));
  ...
  if (!result.languages) {
    const candidates = uniqueLines(lines.filter((l) => {
      if (contactSet.has(l.toLowerCase().trim())) return false;
      return /(arabic|english|urdu|french|hindi|spanish|german|chinese|mandarin|العربية|الانجليزية|الإنجليزية|فرنسي|أردو|هندي)/i.test(l);
    }));
  ...
  if (!result.education) {
    const candidates = uniqueLines(lines.filter((l) => {
      if (contactSet.has(l.toLowerCase().trim())) return false;
      return /(Bachelor|Master|Diploma|PhD|University|College|degree|graduated|بكالوريوس|ماجستير|دبلوم|جامعة|كلية|تخرج)/i.test(l);
    }));
...
export function computeMetrics(s: StructuredResume): AtsMetrics {
  // 1. Structure score — checks section completeness
  const requiredSections: (keyof StructuredResume)[] = ["name", "job_title", "contact", "summary", "experience", "skills", "education"];
  ...
  // Check for consistent date formatting in experience
  const expDates = (s.experience.match(/\b(19|20)\d{2}\b/g) || []).length;
  if (expDates >= 2) structurePoints += 5;
  // Check for section headings (already structured = good)
  structurePoints += 10;
  const structureScore = Math.min(100, structurePoints);

  // 2. Keywords score — action verbs + skills depth
  const actionVerbs = ["led", "managed", "developed", "increased", "reduced", "created", "implemented", "achieved", "designed", "built", "optimized", "delivered", "improved", "launched", "established", "coordinated", "supervised", "analyzed", "streamlined", "mentored", "قاد", "أدار", "طور", "زاد", "أنشأ", "نفذ", "حقق", "صمم", "بنى", "حسن", "أطلق", "أسس", "نسق", "أشرف", "حلل"];
  ...
  const keywordsScore = Math.min(100, verbHits * 8 + (skillItems >= 8 ? 40 : skillItems >= 5 ? 25 : skillItems > 0 ? 10 : 0));

  // 3. Readability score — bullet points, summary quality, consistent formatting
  const bullets = (s.experience.match(/(^|\n)•\s/g) || []).length;
  const expRoles = s.experience.split(/\n{2,}/).filter(Boolean).length;
  let readabilityScore = 0;
  readabilityScore += Math.min(40, bullets * 6); // bullets
  readabilityScore += s.summary.length > 80 ? 25 : s.summary.length > 40 ? 15 : 0; // summary length
  readabilityScore += expRoles >= 2 ? 15 : expRoles >= 1 ? 8 : 0; // multiple roles
  readabilityScore += s.education.trim() ? 10 : 0; // education present
  readabilityScore += s.contact.trim() ? 10 : 0; // contact present
  readabilityScore = Math.min(100, readabilityScore);

  // 4. Impact score — quantified achievements, metrics, percentages
  const numbers = (s.experience.match(/\d+%/g) || []).length;
  const dollarAmounts = (s.experience.match(/\$[\d,]+|\d+[KkMm]\b/g) || []).length;
  const metrics = (s.experience.match(/\d+\s*(users|customers|clients|projects|team|members|people|staff|employees|reports|systems|applications)/gi) || []).length;
  const impactScore = Math.min(100, numbers * 15 + dollarAmounts * 12 + metrics * 10 + verbHits * 5);

  // 5. Completeness score — all fields filled
  const allFields: (keyof StructuredResume)[] = ["name", "job_title", "contact", "summary", "experience", "skills", "education", "certifications", "projects", "languages"];
...
export function buildSuggestions(s: StructuredResume): Suggestion[] {
  ...
  if (!s.name.trim()) tips.push({ id: "name", title: "Missing Name", titleAr: "الاسم غير موجود", description: "Add your full name at the top of your resume.", descriptionAr: "أضف اسمك الكامل في أعلى السيرة.", field: "name", fix: "", actionLabel: "Add Name", actionLabelAr: "أضف الاسم" });
  if (!s.contact.trim()) tips.push({ id: "contact", title: "Missing Contact Info", titleAr: "بيانات الاتصال ناقصة", description: "Add email, phone number, LinkedIn, and location.", descriptionAr: "أضف البريد الإلكتروني ورقم الجوال وLinkedIn والموقع.", field: "contact", fix: "", actionLabel: "Add Contact", actionLabelAr: "أضف بيانات الاتصال" });
  if (s.summary.trim().length < 40) tips.push({ id: "summary", title: "Weak Summary", titleAr: "الملخص المهني ضعيف", description: "Add a concise professional summary (3-5 sentences).", descriptionAr: "أضف ملخصًا مهنيًا مختصرًا (3-5 جمل).", field: "summary", fix: "", actionLabel: "Edit Summary", actionLabelAr: "عدّل الملخص" });
  if (!/\d+%?/.test(s.experience)) tips.push({ id: "metrics", title: "Missing Measurable Impact", titleAr: "لا توجد إنجازات رقمية", description: "Add quantified achievements (e.g., 'Increased sales by 30%').", descriptionAr: "أضف إنجازات رقمية قابلة للقياس (مثال: 'زيادة المبيعات 30%').", field: "experience", fix: "", actionLabel: "Add Metrics", actionLabelAr: "أضف أرقام" });
  if (splitLines(s.skills).length < 5) tips.push({ id: "skills", title: "Skills Need More Depth", titleAr: "قسم المهارات يحتاج تعزيز", description: "Add 8-15 relevant skills for your target role.", descriptionAr: "أضف 8-15 مهارة ذات صلة بالوظيفة المستهدفة.", field: "skills", fix: "", actionLabel: "Add Skills", actionLabelAr: "أضف مهارات" });
  if (!s.education.trim()) tips.push({ id: "education", title: "Education Missing", titleAr: "قسم التعليم ناقص", description: "Add your educational background.", descriptionAr: "أضف خلفيتك التعليمية.", field: "education", fix: "", actionLabel: "Add Education", actionLabelAr: "أضف التعليم" });
  if (!s.certifications.trim()) tips.push({ id: "certifications", title: "No Certifications", titleAr: "لا توجد شهادات", description: "Add professional certifications or relevant courses.", descriptionAr: "أضف الشهادات المهنية أو الدورات.", field: "certifications", fix: "", actionLabel: "Add Certs", actionLabelAr: "أضف شهادات" });
  if (!s.projects.trim()) tips.push({ id: "projects", title: "Projects Missing", titleAr: "قسم المشاريع غير موجود", description: "Add notable projects to showcase your work.", descriptionAr: "أضف مشاريع بارزة لعرض أعمالك.", field: "projects", fix: "", actionLabel: "Add Projects", actionLabelAr: "أضف مشاريع" });
  if (!s.languages.trim()) tips.push({ id: "languages", title: "Languages Missing", titleAr: "قسم اللغات غير موجود", description: "Add languages with proficiency levels.", descriptionAr: "أضف اللغات ومستوى الإتقان.", field: "languages", fix: "", actionLabel: "Add Languages", actionLabelAr: "أضف لغات" });
  ...
  if (expRoles.length > 0) {
    ...
    if (totalBullets < expRoles.length * 3) {
      tips.push({ id: "exp_bullets", title: "Add More Bullet Points", titleAr: "أضف المزيد من النقاط", description: "Each role should have 3-6 achievement-focused bullet points.", descriptionAr: "يجب أن يحتوي كل دور على 3-6 نقاط تركز على الإنجازات.", field: "experience", fix: "", actionLabel: "Edit Experience", actionLabelAr: "عدّل الخبرات" });
    }
  }
  ...
  if (hasWeakVerbs) {
    tips.push({ id: "weak_verbs", title: "Replace Weak Verbs", titleAr: "استبدل الأفعال الضعيفة", description: "Replace 'Responsible for' with action verbs like 'Led', 'Managed', 'Developed'.", descriptionAr: "استبدل 'مسؤول عن' بأفعال قوية مثل 'قاد'، 'أدار'، 'طوّر'.", field: "experience", fix: "", actionLabel: "Improve Experience", actionLabelAr: "حسّن الخبرات" });
  }
...
function buildExperienceHtml(content: string): string {
  ...
  return groups
    .map((group) => {
      const lines = group.split("\n").map((l) => cleanArtifacts(l)).filter(Boolean);
      if (!lines.length) return "";
      const header = lines[0];
...
export function structuredToHtml(s: StructuredResume, lang: string): string {
  ...
  for (const key of SECTION_ORDER) {
    ...
    if (key === "summary") parts.push(buildParagraphs(splitLines(content)));
      ...
      const items = splitLines(content).map((x) => x.replace(/^[•▪*\-]\s*/, "").trim()).filter(Boolean);
...
function parseExperienceSection(sectionEl: Element): string {
  ...
  if (roleBlocks.length) {
    const groups = roleBlocks.map((block) => {
      const header = cleanArtifacts(block.querySelector('[data-role-header="true"]')?.textContent || "") || cleanArtifacts(block.querySelector("p,strong")?.textContent || "");
      const bullets = Array.from(block.querySelectorAll("li")).map((li) => cleanArtifacts(li.textContent || "")).filter(Boolean).map((txt) => `• ${txt}`);
      return [header, ...bullets].filter(Boolean).join("\n");
    });
...
function parseListSection(sectionEl: Element): string {
  const items = Array.from(sectionEl.querySelectorAll("li")).map((li) => cleanArtifacts(li.textContent || "")).filter(Boolean);
  if (items.length) return items.join("\n");
  return parseSectionChildrenToLines(sectionEl).map((x) => x.replace(/^[•▪*\-]\s*/, "").trim()).filter(Boolean).join("\n");
}
...
export function htmlToStructured(html: string, lang: string): StructuredResume {
  ...
  for (const key of SECTION_ORDER) {
    ...
    else if (key === "skills" || key === "certifications" || key === "projects" || key === "languages") result[key] = parseListSection(sectionEl);
    else result[key] = parsePlainSection(sectionEl);
  }

  // Fallback: parse unstructured HTML
  ...
  if (!hasAnyData) {
    ...
    for (const node of Array.from(doc.body.childNodes)) {
      ...
      if (tag === "hr") { headerDone = true; currentSection = null; continue; }
      if (!headerDone) {
        if ((tag === "h1" || tag === "h2") && !gotName) {
          const maybeField = labelToField[text.toLowerCase()] || labelToField[text];
          if (maybeField) { currentSection = maybeField; headerDone = true; }
          else { result.name = text; gotName = true; }
          continue;
        }
        if (tag === "h3" && gotName && !gotTitle) { result.job_title = text; gotTitle = true; continue; }
        if ((tag === "p" || tag === "div") && gotName) { result.contact += (result.contact ? "\n" : "") + text; continue; }
      }
      if (tag === "h2" || tag === "h3") {
        const key = text.toLowerCase();
        const matchedField = labelToField[key] || labelToField[text];
        if (matchedField) { currentSection = matchedField; headerDone = true; continue; }
      }
