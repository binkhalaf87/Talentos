...
interface UseInterviewSessionReturn {
  ...
  submitAnswer: (transcript: string, jobTitle?: string, cvSummary?: string, language?: string) => Promise<AnswerEvaluation | null>;
  nextQuestion: () => void;
...
export function useInterviewSession(): UseInterviewSessionReturn {
  ...
  const endSession = useCallback(async () => {
    ...
    const summary = {
      ...
      strongest: answers.reduce((best, a) =>
        (!best || (a.evaluation?.score ?? 0) > (best.evaluation?.score ?? 0) ? a : best),
        answers[0]
      )?.questionText,
      weakest: answers.reduce((worst, a) =>
        (!worst || (a.evaluation?.score ?? 0) < (worst.evaluation?.score ?? 0) ? a : worst),
        answers[0]
      )?.questionText,
    };
